...
- **`VISUALIZATION_GUIDE.md`** — the chart-choice decision framework (data shape → encoding) and chart
  catalog (when to use / when not / the anti-pattern). *Not created yet — planned in M1.*
...
This repo is at **M0 stage** but the environment is set up and working. What exists:
...
1. Answer the chart-choice checklist in `VISUALIZATION_GUIDE.md`. State the chart type and WHY in one line.
   (Until that file exists, use the M1 decision framework in `PLAN.md`.)
...
- Grey for context, ONE accent for the point. Thousands separators + unit-aware tick formatters always.
